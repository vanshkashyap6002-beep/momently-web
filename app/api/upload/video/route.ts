...
/** POST /api/upload/video — multipart/form-data: { file, projectId } */
export async function POST(request: NextRequest) {
  return toRouteResponse(async () => {
    ...
    const file = parseOrThrow(mediaFileSchema("VIDEO"), formData.get("file"));

    return mediaService.uploadVideo(userId, projectId, file);
  }, 201);
}
