...
import { createDraftTemplate, updateDraftTemplate } from "@/app/actions/community-template.actions";
...
export function CreatorTemplateForm({ template }: { template?: Template }) {
  ...
  const {
    ...
    defaultValues: template
      ...
      : { title: "", category: "", thumbnail: "", description: "", isPremium: false, price: 0 },
  });
  ...
  async function onSubmit(values: FormValues) {
    ...
    const payload = {
      ...values,
      price: values.isPremium ? Number(values.price) : 0,
      previewImages: [],
    };
    const result = template
      ? await updateDraftTemplate(template.id, payload)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-xl space-y-4">
      ...
      <label className="block">
        ...
        <textarea rows={4} {...register("description", { required: "Description is required" })} className={inputClass} />
        {errors.description && (
      ...
      {isPremium && (
        <label className="block">
          ...
          <input type="number" min={0} step="1" {...register("price", { valueAsNumber: true })} className={inputClass} />
        </label>
      )}
